...
// INGLES
#define T_FACIL_EN       26   // Easy mode selected
#define T_MEDIO_EN       27   // Medium mode selected
#define T_DIFICIL_EN     28   // Hard mode selected

#define T_NUEVO_EN       29   // New pattern
#define T_INTENTA_EN     30   // Try again
#define T_IDIOMA_EN      31   // English selected

#define T_ACIERTO_EN     35   // Good job!
#define T_VICTORIA_EN    36   // Congratulations!
#define T_APLAUSOS       37   //Hurra!!

#define T_COLOR_ROJO_EN  32   // Red
#define T_COLOR_AZUL_EN  33   // Blue
#define T_COLOR_VERDE_EN 34   // Green
...
// PULSANTES
#define BUTTON_PIN        2      // cambio patrón / modo
#define BUTTON_IDIOMA     3      // cambio idioma y mute

// CONEXION BLUETOOTJ
#define BT Serial1   // HC-05: TX→19, RX→18 (con divisor)
...
int seccionColor[9];   // patrón del tablero 1
...
// DETECCION DE COLOR ------- DONDE EL GUAGUA PONE LOS COLORES CON BORNERAS BANANAS
// Casillas 0..8
// ROJO -> A a GND
// AZUL -> B a GND
// VERDE -> A y B a GND
...
void setup() {
  ...
  // Entradas del tablero del guagua
  for (int i = 0; i < 9; i++) {
...
// 
// BOTÓN DE IDIOMA: CICLO ES → KQ → COMB → EN + MUTE/LARGO
// 
...
// 
// ACTUALIZAR TABLERO DEL GUAGUA: ACIERTO / ERROR / VICTORIA
// 
void actualizarTableroJugador() {
  ...
  for (int i = 0; i < 9; i++) {
    ...
    if (jugadorColor[i] != ultimoJugadorColor[i]) {
      ...
      // Si la casilla ya fue correcta antes → ignorar cambios
      if (casillaCorrecta[i]) continue;
...
if (!victoriaAnunciada) {
  ...
  if (modo == 0 || modo == 1) {   // fácil o medio
     reproducirAplausos();
     } else {                        // difícil
    reproducirVictoria();
  }
...
void reproducirAplausos() {
  ...
  delay(1500);   // dejar terminar el audio anterior
  mp3.play(T_APLAUSOS);
}